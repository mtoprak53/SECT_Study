...
class UserViewsTestCase(TestCase):
    ...
    def test_delete_post(self):
        with app.test_client() as client:
            ...
            # print(f"/posts/{self.post.id}/delete")   # test
            html = resp.get_data(as_text=True)
            self.assertEqual(resp.status_code, 200)   # ALWAYS 404
            self.assertNotIn(f"Test Post", html)
            self.assertIn(f"Test User</h3>", html)