...
def signup():
    ...
    if g.user:
        flash("Please sign out first, to register another user!!", "danger")
        return redirect("/")
...
def logout():
    ...
    do_logout()
    flash(f"Bye bye {user.username}", 'success')
...
def show_likes(user_id):
    """Show the liked messages of this user."""
    ...
    # solution uses relationship -->  likes=user.likes
    user_like_ids = [like.id for like in user.likes]
    likes = (Message
                .query
...
def delete_user():
    ...
    # # Does not work in unittest
    # db.session.delete(g.user)

    # Does work in unittest
...
def like_warble(msg_id):
    """Add the warble to the liked list."""

    like = (Likes.query
                 .filter_by(user_id=g.user.id, message_id=msg_id)
                 .first())                
...
def homepage():
    ...
    if g.user:
        user_following_ids = [u.id for u in g.user.following] + [g.user.id]
